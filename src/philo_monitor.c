...
void print_status_message(char *message, t_philosopher *philosopher, int philosopher_id)
{
    size_t current_time;
...
int is_philosopher_dead(t_philosopher *philosopher, size_t time_to_die)
{
    ...
    if (get_current_time() - philosopher->last_meal >= time_to_die && philosopher->is_eating == 0)
...
{
    t_philosopher *philosophers = (t_philosopher *)pointer;
    while (1)
        if (check_philosophers_status(philosophers) == 1 || check_if_all_philosophers_ate(philosophers) == 1)
            break;
    return (pointer);
}